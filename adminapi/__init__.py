def setup_admin_api(app, db, db_session):
    for name, model in db.entities.items():
        ...
        app.add_api_route("/api/admin/{}".format(name.lower()), make_list_endpoint(model))
        ...
        app.add_api_route("/api/admin/%s/{id}" % name.lower(), make_detail_endpoint(model))
...
""".format(kw=kw, kwt=kwt), d)
        ...
        app.add_api_route("/api/admin/%s" % name.lower(), make_create_endpoint(model), methods=["POST"])
